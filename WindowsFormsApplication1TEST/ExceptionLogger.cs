...
namespace Utilities
{
  ...
  public abstract class LoggerImplementation
  {
    ...
    public abstract void Logging(string _log);
  }
  ...
  public class ExceptionLogger
  {
    ...
    public void LogData(string _logData)
    {
        StringBuilder logData  = new StringBuilder();

        logData.AppendLine("===========================================================");
        logData.AppendLine("Application:       " + Application.ProductName);
        logData.AppendLine("Version:           " + Application.ProductVersion);
        logData.AppendLine("Date:              " + DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss"));
        logData.AppendLine("Computer name:     " + SystemInformation.ComputerName);
        logData.AppendLine("User name:         " + SystemInformation.UserName);
        logData.AppendLine("OS:                " + Environment.OSVersion.ToString());
        logData.AppendLine("Culture:           " + CultureInfo.CurrentCulture.Name);
        logData.AppendLine("Resolution:        " + SystemInformation.PrimaryMonitorSize.ToString());
        logData.AppendLine("System up time:    " + GetSystemUpTime());
        logData.AppendLine("App up time:       " +
          (DateTime.Now - Process.GetCurrentProcess().StartTime).ToString());

        MEMORYSTATUSEX memStatus = new MEMORYSTATUSEX();
        if (GlobalMemoryStatusEx(memStatus))
        {
            logData.AppendLine("Total memory:      " + memStatus.ullTotalPhys / (1024 * 1024) + "Mb");
            logData.AppendLine("Available memory:  " + memStatus.ullAvailPhys / (1024 * 1024) + "Mb");
        }

        logData.AppendLine("");
        
        logData.AppendLine(_logData);
        logData.AppendLine("");
        ...
        for (int i = 0; i < loggers.Count; i++)
        {
            loggers[i].Logging(logData.ToString());
        }
    }
  }
}
