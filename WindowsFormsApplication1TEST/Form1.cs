...
namespace WindowsFormsApplication1TEST
{
    public partial class Form1 : Form
    {
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            XDocument xml = XDocument.Load("DynamicConfiguration.xml");

            //XDocument cpo = XDocument.Load("DynamicConfiguration.xml");
            ...
            String filepath = Application.StartupPath + @"\AccountBalanceLinqTest.cs";

            TxtFile txtObj = new TxtFile();
            txtObj.FileName = filepath;


            List<XElement> messages = xml.Root.Descendants("message").Where(d => d.Attribute("name").Value == "Account Balance Enq").Descendants("request").Descendants("properties").Elements().ToList();


            //IEnumerable<XElement> messages = methodName.Elements("request").Descendants("property");
            ...
            alC.Add("using System.Threading.Tasks;");
            alC.Add(" ");
            string c = "public class AccountBalanceLinqTest";
            alC.Add(c);
            alC.Add("{");
            ...
            setValues.Add("");
            setValues.Add("");
            setValues.Add(" public void SetDefaultValue()");
            setValues.Add("{");
            ...
            {
                string propertyName = message.Attribute("name").Value;
                string defaultValue = message.Attribute("defaultValue").Value;

                string propertyNameWithoutBadChars = RemoveBadChar(propertyName);

                string resCs = "public string " + propertyNameWithoutBadChars + GetSet;

                //adding property list to main array
                alC.Add(resCs);

                string setProperty = propertyNameWithoutBadChars  + "=" + "\""+ defaultValue + "\"";
                
                //set properties to default in second array
                setValues.Add(setProperty);
                
            
               //String map =  propertyName + "|" + defaultValue;
            ...
            //adding second array to the main array
            //String[] setValuesString = (string[])setValues.ToArray();
            ...
            String[] arrayToString = alC.ToArray(typeof(string)) as string[];

            txtObj.ArrayToFile(arrayToString);

            //File.WriteAllLines(filepath, alC.Cast<string>());
